...
class helper
{
    ...
    public static function getStoreStatus($vendor_id)
    {
        ...
        foreach ($timings as $timing) {
            if ($timing['day'] === $currentDay) {
                ...
                $openTime = Carbon::createFromFormat('h:i A', $timing['open_time']);
                $closeTime = Carbon::createFromFormat('h:i A', $timing['close_time']);
        ...
        return $statusMapping[$storeStatus];
    }

    public static function getStoreTimings($vendorId)
    {
        date_default_timezone_set(self::appdata($vendorId)->timezone);
        $timings = Timing::where('vendor_id', $vendorId)->get();

        $formattedTimings = [];
        foreach ($timings as $timing) {
            if ($timing->is_always_close == 1) {
                $formattedTimings[$timing->day] = 'Cerrada';
            } else {
    ...
    public static function vendorinfo($vendor = null)
    {
        ...
        $vendorinfo = Settings::where('id', 2)->first();
        return $vendorinfo;
    }
    public static function storeinfo($vendor = null)
    {
        // Change made by Hadi to set store page as default home page
        // $vendorinfo = User::where('slug', $vendor)->first();
        $vendorinfo = User::where('id', 2)->first();
        return $vendorinfo;
    }
    ...
    public static function createorder($vendor,$user_id,$session_id,$payment_type_data, $payment_id, $customer_email, $customer_name, $customer_mobile, $stripeToken, $grand_total, $delivery_charge, $address, $building, $landmark, $postal_code, $discount_amount, $sub_total, $tax, $delivery_time, $delivery_date, $delivery_area, $couponcode, $order_type, $notes , $table_id)
    {
        try {
            ...
            { 
                ...
                //     $payment_type = 2;
                // }
                // if ($payment_type_data == 13) {
                //     $payment_type = 3;
                // }
                // if ($payment_type_data == 14) {
                //     $payment_type = 4;
                // }
                // if ($payment_type_data == 15) {
                //     $payment_type = 5;
                // }
                // if ($payment_type_data == 16) {
                //     $payment_type = 7;
                // }
                // if ($payment_type_data == 17) {
                //     $payment_type = 8;
                // }
                // if ($payment_type_data == 18) {
                //     $payment_type = 9;
                // }
...
{store_url}";
    ...
    {
        $data = ['title' => "Order Invoice", 'order_type' => $order_type, 'customer_email' => $customer_email, 'customer_name' => $customer_name, 'company_email' => $companyemail, 'company_name' => $companyname, 'order_number' => $order_number, 'grand_total' => $grand_total, 'trackurl' => $trackurl];
    ...
    public static function whatsappmessage($order_number, $vendor_slug, $vendordata)
    {
        ...
        $payment_type = helper::payment_method_name($orderdata->payment_type);
