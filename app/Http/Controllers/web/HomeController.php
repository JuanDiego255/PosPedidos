...
class HomeController extends Controller
{
    ...
    public function save_contact(Request $request)
    {
        ...
        helper::vendor_contact_data($vendordata->name, $vendordata->email, $request->first_name . " " . $request->last_name, $request->email, $request->mobile, $request->message);
        return redirect()->back()->with('success', trans('messages.success'));
    }
    ...
    public function addtocart(Request $request)
    {
        try {
            ...
            if (Auth::user()) {
                $totalcart = helper::getcartcount($request->vendor_id, Auth::user()  ? Auth::user()->id : "");
            } else {
                $totalcart = helper::getcartcount($request->vendor_id, '');
            }

            $cartdata = $this->cartdata($request)['cartdata'];
            $storeinfo = $this->cartData($request)['storeinfo'];

            $cartView = view('front.partials.cart', compact('cartdata', 'storeinfo'));
            $cartHTML = $cartView->render();
    ...
    public function cartData(Request $request)
    {
        ...
        return ['cartdata' => $cartdata, 'storeinfo' => $storeinfo];
    }
    ...
    public function checkout(Request $request)
    {
        ...
        $cartdata = $cartitems->get();
        if (count($cartdata) == 0) {
            return redirect('/cart')->with('error', trans('messages.cart_empty'));
        }
    ...
    public function paymentmethod(Request $request)
    {
        try {
            ...
            $url = URL::to("/success/" . $orderresponse);
            if (Auth::user()) {
                $user = User::findOrfail($user_id);
                if ($request->sub_total > 5000 && $request->sub_total < 7000) {
                    $user->points = $user->points + 50;
                } else if ($request->sub_total > 7000 && $request->sub_total < 15000) {
                    $user->points = $user->points + 130;
                }
                $user->update();
            }
            ...
            return response()->json(['status' => 1, 'message' => $th->getMessage(), "order_number" => $orderresponse, "url" =>  $url], 200);
        }
    }

    public function ordersuccess_new(Request $request)
    {
        $host = $_SERVER['HTTP_HOST'];
        if ($host == env('WEBSITE_HOST')) {
            $storeinfo = helper::storeinfo($request->vendor);
        ...
        $whmessage = helper::whatsappmessage($order_number, $storeinfo->slug, $storeinfo);

        $contact = helper::appdata($storeinfo->id)->contact;

        $whatsappUrl = "https://api.whatsapp.com/send?phone={$contact}&text=" . $whmessage;
    ...
    public function ordersuccess(Request $request)
    {
        ...
        $whatsappUrl = "https://api.whatsapp.com/send?phone={$contact}&text=$whmessage";
        return view('front.ordersuccess2', compact('storeinfo', 'order_number', 'whmessage', 'whatsappUrl'));
    }
