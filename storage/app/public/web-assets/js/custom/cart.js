...
function addtocart(id,name,price,image,tax,qty,orignal_price) {
       "use strict";
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#cartcount2').html(response.totalcart);
                $('#cartcount3').html(response.totalcart);
                $('#cartcount_mobile').html(response.totalcart);
                console.log(response.cartdata);
                $('#floating_cart_container').html(response.cartdata); // Replace 'yourCartContainer' with the actual ID or class of your cart container
 ...
 {
    "use strict";
    var message = 'Estoy interesado en este producto : ';
    $.ajax({
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        ...
        success: function (response) {  
           ...
           var whatsappnumber = $('#vendor_contact').val();
