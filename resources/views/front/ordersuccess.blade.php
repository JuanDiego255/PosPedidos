            ...
            <div class="row justify-content-between d-flex align-items-md-center vh-100">
                <div class="col-md-12 col-lg-6 order-2 order-lg-0 successfully-contain">
                    ...
                    <div class="form-control m-auto  rounded-2 d-flex p-0 rounded-5">
                        <input type="text" class="form-control input-h border-0  rounded-5" value="{{ URL::to( $host  ==  env('WEBSITE_HOST') ? '/track-order/' . $order_number : '' . '/track-order/' . $order_number) }}" id="data">
                        <a href="{{ URL::to( $host  ==  env('WEBSITE_HOST') ?  '/track-order/' . $order_number : '/track-order/' . $order_number) }}" class="copy-btn px-5 rounded-5 text-dark" target="_blank">
                    ...
                    <div class="row mt-5">
                        <div class="col-md-4 col-lg-4 col-xl-4 mb-2 mb-lg-2 mb-xl-0">
                            <a class="btn-primary w-100 text-center my-cart-account-btn" href="{{ URL::to($host  ==  env('WEBSITE_HOST') ? '' : '') }}">{{ trans('labels.continue_shop') }}</a>
                        </div>
                        <div class="col-md-4 col-lg-4 col-xl-4 mb-2 mb-lg-2 mb-xl-0">
                            ...
                            App\Models\SystemAddons::where('unique_identifier', 'telegram_message')->first()->activated == 1)
                                ...
                                @if($telegram_message == 1)
                                    <a href="{{ URL::to('/telegram/' . $order_number . '') }}"
                        ...
                        <div class="col-md-4  btn-container">
                              ...
                              <a id="whatsapp_button" href="https://api.whatsapp.com/send?phone={{ helper::appdata($storeinfo->id)->contact }}&text={{ $whmessage }}" class=" ">
                                
                                <button><i class="fab fa-whatsapp me-1"></i>{{ trans('labels.send_order_whatsapp') }}</button>
